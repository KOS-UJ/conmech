...
def test_emit_table_writes_latex_and_csv(tmp_path, capsys):
    headers = ["$h$", r"$\|e\|_{L^2}$", "rate"]
    rows = [
        ["$1/4$", table_exporter.format_error(1.5e-2), table_exporter.format_rate(float("nan"))],
        ["$1/8$", table_exporter.format_error(3.75e-3), table_exporter.format_rate(2.0)],
    ]
    path = table_exporter.export_table(
        "X", headers, rows, caption="c", label="tab:x", outputs_path=str(tmp_path)
    )

    output = capsys.readouterr().out
    assert output.count("\\begin{table}") == output.count("\\end{table}") == 1
    body = output.split("\\begin{tabular}")[1].split("\\end{tabular}")[0]
    assert body[body.index("{") + 1 : body.index("}")].count("c") == len(headers)
    for line in body.splitlines():
        if line.endswith("\\\\"):
            assert line.count("&") == len(headers) - 1, line

    lines = Path(path).read_text(encoding="utf-8").splitlines()
    assert lines[0].split(",") == headers
    assert len(lines) == 1 + len(rows)
    assert "1.500e-02" in lines[1] and "--" in lines[1]
...
def test_errors_module_imports_without_conmech():
    """`errors` must not need conmech, scipy or matplotlib at import time."""
    script = textwrap.dedent(
        """
        import sys

        class Blocker:
            BLOCKED = ("conmech", "scipy", "matplotlib")
        ...
        from examples.common import errors
        assert errors._TRI_RULE_DEG5.shape == (7, 4)
        assert "scipy" not in sys.modules and "conmech" not in sys.modules
        print("OK")
        """
    )
