...
class State:
    ...
    def __init__(self, grid):
        self.grid = grid
        self.displacement: np.ndarray = np.zeros((self.grid.indNumber(), 2))
    ...
    def set_u_and_displaced_points(self, displacement_vector: np.ndarray):
        ...
        self.displaced_points[:self.grid.indNumber(), :2] = \
            self.grid.Points[:self.grid.indNumber(), :2] + self.displacement[:, :2]
