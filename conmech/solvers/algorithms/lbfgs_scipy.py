...
def _minimize_lbfgsb(
    ...
    **unknown_options,
    ...
    # func = MemoizeJac(fun)
    # sf = _prepare_scalar_function(func, x0, jac=func.derivative, args=args, epsilon=eps,
    #                               bounds=new_bounds,
    #                               finite_diff_rel_step=finite_diff_rel_step)

    func_and_grad = lambda x: fun(x, args[0])  # sf.fun_and_grad ###
    ...
    # print("zeros: ", time.time() - t_start)
    task[:] = "START"

    n_iterations = 0

    # x_old = array(x0, float64)
    # x_old[:] = x0
    # start = True
    while 1:
        ...
        if task_str.startswith(b"FG"):
            # if not start and np.linalg.norm(x - x_old, ord = np.inf) < ftol:
            #     warnflag = 0
            #     break
            # start = False
            # x_old[:] = x
