...
def print_dataset(dataset, cutoff, timestamp, description):
    ...
    for i in range(iterations):
        ...
        #        size=(setting.points_number, config.DIM)
        #    ))
...
def print_multiple_dynamic(get_a_function, description=None):
    ...
    plotter = Plotter()
    axs = plotter.get_multiple_axs(plots, timesteps)
    unit = config.DRAW_FORCE_UNIT

    print_steps(plotter, axs, 0, timesteps, skip, [0.0, unit], get_a_function)
    print_steps(plotter, axs, 1, timesteps, skip, [0.0, -unit], get_a_function)
    print_steps(plotter, axs, 2, timesteps, skip, [unit, 0.0], get_a_function)
    print_steps(plotter, axs, 3, timesteps, skip, [-unit, 0.0], get_a_function)
...
def print_steps(drawer, axs, row, timesteps, skip, force, get_a_function):
    # TODO: Get data generated by Data
    setting = SolverTorch(mesh_type=config.MESH_TYPE, mesh_density=config.MESH_DENSITY)

    forces = np.repeat([force], setting.points_number, axis=0)
    setting.set_forces(forces)

    for time in range(timesteps * skip):
...
def draw_one_dynamic(self, model, description):
    ...
    forces_at_time = torch.repeat_interleave(force, mesh.points_number, axis=0)
