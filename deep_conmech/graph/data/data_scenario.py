...
def generate_scenario_data_process(
    dataset, all_scenarios, num_workers, process_id
):
    ...
    if not indices_to_do:
        step_tqdm = basic_helpers.get_tqdm(range(1), desc=f"Process {process_id} - done", position=process_id)
        return True
    ...
    for scenario in assigned_scenarios:
        ...
        step_tqdm = basic_helpers.get_tqdm(
            range(1, config.VAL_PRINT_EPISODE_STEPS + 1), desc=tqdm_description, position=process_id
        )
    ...
    def generate_all_data(self):
        ...
        while result is False:
            result = basic_helpers.run_processes(
                generate_scenario_data_process,
                (self, self.all_scenarios),
                num_workers,
            )
            if result is False:
