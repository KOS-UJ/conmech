...
def get_alpha(x, p1, p2, p3):
    return (
        (p2[1] - p3[1]) * (x[0] - p3[0]) + (p3[0] - p2[0]) * (x[1] - p3[1])
    ) / (
...
def get_beta(x, p1, p2, p3):
    return (
        (p3[1] - p1[1]) * (x[0] - p3[0]) + (p1[0] - p3[0]) * (x[1] - p3[1])
    ) / (
        (p2[1] - p3[1]) * (p1[0] - p3[0]) + (p3[0] - p2[0]) * (p1[1] - p3[1])
    )
...
def approximate_one(new_point, old_points, old_values, old_cells):
    ...
    v1, v2, v3 = old_values[closest_cell]
    return alpha * v1 + beta * v2 + gamma * v3



#@njit
def approximate_all(new_points, old_points, old_values, old_cells):
    new_values = np.zeros_like(new_points)

    for i in range(len(new_points)):
        new_point = new_points[i]
        new_value = approximate_one(new_point, old_points, old_values, old_cells)
