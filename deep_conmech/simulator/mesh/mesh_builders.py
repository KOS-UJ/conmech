...
def build_mesh(mesh_type, mesh_density, scale, is_adaptive, create_in_subprocess):
    if mesh_type == "cross":
        function = lambda: get_cross_rectangle_new(mesh_density, scale)
    elif mesh_type == "meshzoo":
        function = lambda: get_meshzoo_rectangle(mesh_density, scale)
    elif mesh_type == "dmsh":
        function = lambda: get_dmsh_rectangle(mesh_density, scale)
    elif "pygmsh" in mesh_type:
        inner_function = lambda: get_pygmsh(mesh_type, mesh_density, scale, is_adaptive)
        function = (
...
def get_pygmsh(type, mesh_density, scale, is_adaptive):

    with pygmsh.geo.Geometry() as geom:
        if "rectangle" in type:
            poly = geom.add_polygon(
                [[0.0, 0.0], [0.0, scale], [scale, scale], [scale, 0.0]]
            )
        elif "circle" in type:
            geom.add_circle([scale / 2.0, scale / 2.0], scale / 2.0)
        elif "polygon" in type:
            geom.add_polygon(
                [
                    [scale * 0.0 / 1.4, scale * 0.2 / 1.4],
                    [scale * 1.0 / 1.4, scale * 0.0 / 1.4],
                    [scale * 1.1 / 1.4, scale * 1.4 / 1.4],
                    [scale * 0.1 / 1.4, scale * 0.9 / 1.4],
                ]
            )
        elif "spline" in type:
            # lcar = 0.1
            p1 = geom.add_point([0.0, 0.0])
            p2 = geom.add_point([scale, 0.0])
            p3 = geom.add_point([scale, scale / 2.0])
            p4 = geom.add_point([scale, scale])
            s1 = geom.add_bspline([p1, p2, p3, p4])

            p2 = geom.add_point([0.0, scale])
            p3 = geom.add_point([scale / 2.0, scale])
        ...
        set_mesh_size(geom, mesh_density, scale, is_adaptive)
...
def get_cross_rectangle_new(mesh_density, scale):
    return get_cross_rectangle(
        mesh_size_x=mesh_density,
        mesh_size_y=mesh_density,
        corners=scale * np.array([0.0, 0.0, 1.0, 1.0]),
    )
...
def get_cross_rectangle(mesh_size_x, mesh_size_y, corners):
    ...
    min = corner_min(corners)
    edge_len_x = basic_helpers.len_x(corners) / size_x
    edge_len_y = basic_helpers.len_y(corners) / size_y
