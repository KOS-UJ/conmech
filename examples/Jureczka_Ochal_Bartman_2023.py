...
class QuasistaticSetup(Quasistatic):
    ...
    ze_coef: ... = 2
    time_step: ... = 1/32
    contact_law: ... = JureczkaOchalBartman2023
...
def main(show: bool = True, save: bool = False):
    ...
    config = Config()
    for state in states:
        stress = constitutive_law(state.displacement, state.velocity, setup, state.body.mesh.elements,
                                  state.body.mesh.initial_nodes)  # + constitutive_law(w_xi, w_yi, mi_prime, lambda_prime, dx, dy)
        c = np.linalg.norm(stress, axis=(1, 2))
        state.temperature = stress[:, 1, 0]
        drawer = Drawer(state=state, config=config)
        drawer.node_size = 0
        drawer.draw(show=show, temp_min=min(state.temperature), temp_max=max(state.temperature), save=save)
